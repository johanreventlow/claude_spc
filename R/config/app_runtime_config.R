...
initialize_runtime_config <- function(override_options = list(), use_environment_profiles = TRUE) {
  log_debug("Initializing centralized runtime configuration", "RUNTIME_CONFIG")

  # Choose configuration approach
  if (use_environment_profiles) {
    log_debug("Using environment profiles for configuration", "RUNTIME_CONFIG")

    # Environment detection
    environment_type <- determine_environment_type_from_context(override_options)
    log_debug(paste("Detected environment type:", environment_type), "RUNTIME_CONFIG")
    ...
    log_debug("Using legacy configuration approach", "RUNTIME_CONFIG")
  ...
  log_debug("✅ Runtime configuration initialized successfully", "RUNTIME_CONFIG")
  return(config)
}
...
apply_runtime_config <- function(config) {
  log_debug("Applying runtime configuration to global variables", "CONFIG_APPLY")
  ...
  log_debug("✅ Runtime configuration applied to global variables", "CONFIG_APPLY")
  return(invisible(TRUE))
}
...
convert_profile_to_legacy_config <- function(profile) {
  log_debug("Converting environment profile to legacy config format", "CONFIG_CONVERT")
  ...
  log_debug("✅ Environment profile converted to legacy format", "CONFIG_CONVERT")
  return(config)
}